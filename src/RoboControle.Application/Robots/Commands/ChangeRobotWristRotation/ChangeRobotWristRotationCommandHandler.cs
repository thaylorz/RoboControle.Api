...
public sealed class ChangeRobotWristRotationCommandHandler(IRobotsRepository _robotsRepository) : IRequestHandler<ChangeRobotWristRotationCommand, ErrorOr<RobotResult>>
{
    public async Task<ErrorOr<RobotResult>> Handle(ChangeRobotWristRotationCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = arm.Wrist.ChangeRotationState(request.Rotation);
